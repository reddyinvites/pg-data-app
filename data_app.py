...
def reset_form():
    ...
    st.session_state.sharing = 2
    st.session_state.total_beds = 1
    ...
    if "area" in st.session_state:
        st.session_state.area = list(area_locality_map.keys())[0] if area_locality_map else ""

    if "locality" in st.session_state:
...
if st.button("💾 Save Room"):
    st.session_state.saved_rooms.append({
        "floor":floor,
        "room_no":room_no,
        "sharing":sharing,
